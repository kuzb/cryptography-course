...
def mostFrequentLetter(string):
  # Creates a list of tuples s.t. (letter, frequency), then find the tuple where frequency is the highest(while ignoreing space)
...
def encryptCharAffine(plaintext, a, b, n, onlyUppercase):
  if onlyUppercase:
    ...
    return plaintext if ord(plaintext) > 90 and ord(plaintext) < 90 else chr(((a * (ord(plaintext) - 65) + b) % n) + 65)
...
def decryptCharAffine(ciphertext, a, b, n, onlyUppercase):
  if onlyUppercase:
    # used for alphabets 1-25
    return ciphertext if ord(ciphertext) > 90 and ord(ciphertext) < 90 else chr(((a * ((ord(ciphertext) - 65) - b )) % n) + 65) 
